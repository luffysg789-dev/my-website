...
test('piano html includes header, keyboard shell, orientation hint, and tip mount', () => {
  ...
  assert.match(html, /\/piano\/style\.css\?v=20260328-03/);
  assert.match(html, /\/games-config\.js\?v=20260328-03/);
  assert.match(html, /\/piano\/script\.js\?v=20260328-03/);
...
test('piano script prepares note playback and keeps mobile piano portrait-locked visually', () => {
  const html = fs.readFileSync(htmlPath, 'utf8');
  const css = fs.readFileSync(cssPath, 'utf8');
  const js = fs.readFileSync(jsPath, 'utf8');
